...
Run the tests (CPU then GPU). GPU Make targets skip with `WARN` if `nvcc` is missing. `make test-gpu` uses Slurm like `make bench-gpu` when the controller answers; otherwise it warns and runs locally. Same seed: bit-identical dumps for one mode twice; naive vs opt within `EPS` (default `1e-4`).
...
python3 scripts/replay.py cpu_naive.csv cpu_opt.csv
```

Record then replay (`rec`, 500 agents, 5000 steps):
...
Quick table (N=100, steps=100). Naive modes skip when N > 20000. Sweeps write `results/bench_results.csv`.
